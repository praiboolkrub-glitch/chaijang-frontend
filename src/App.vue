<template>
  <div class="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 sm:py-8">
    <div class="mx-auto w-full max-w-screen-xl">
      <header
        class="mb-6 rounded-3xl bg-gradient-to-r from-indigo-600 to-sky-500 px-6 py-8 sm:px-8 sm:py-10 text-white shadow-xl"
      >
      ...
      <div class="space-y-8">
        <div
          v-if="!loginChecked"
          class="rounded-3xl bg-white p-6 text-slate-600 shadow-sm"
        >
          {{ statusMessage }}
        </div>
        ...
        <div v-else>
          ...
          <div
            v-else-if="currentPage === 'expense' || currentPage === 'income'"
          >
            <div
              class="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-3xl bg-white p-4 shadow-sm"
            >
              <div>
...
const visiblePages = computed(() =>
  pages.filter(
    (page) =>
      page.key !== "users" ||
      !currentUser.value?.line_mid ||
      !currentUser.value?.household_id
  )
);
...
const loadData = async () => {
  try {
    const [categoriesResponse, bankResponse, householdBankResponse] =
      await Promise.all([
        fetchCategories(),
        currentUser.value?.id
          ? fetchBankAccounts(currentUser.value.id)
          : Promise.resolve({ success: true, data: [] }),
        currentUser.value?.household_id
          ? fetchBankAccounts(undefined, currentUser.value.household_id)
          : Promise.resolve({ success: true, data: [] }),
      ]);
...
const headerExpenseTotal = computed(() =>
  transactions.value.reduce(
    (sum, item) =>
      sum + (item.transaction_type === "expense" ? Number(item.amount) : 0),
    0
  )
);
  ...
  try {
    ...
    if (response.success && response.data) {
      const userData = {
        ...response.data,
        display_name:
          response.data.display_name || lineProfile.value.displayName,
        profile_picture:
          response.data.profile_picture || lineProfile.value.pictureUrl,
      };
      if (response.data.household_id) {
        currentUser.value = userData;
        statusMessage.value = `ยินดีต้อนรับ ${
